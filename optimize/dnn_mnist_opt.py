...
def FindMaximalRobustness(optimization_problem_type, input_id):
  ...
  for s in range(l3_n - 1):
    ...
    InY = [ solver.NumVar(0.0, 1.0, 'y%s' % i) for i in range(l0_n) ]
    delta = solver.NumVar(0.0, infinity, 'delta')

    solver.Minimize(delta)

    # Create input layer constraints
    input_cond = [None] * l0_n * 2
    for i in range (l0_n):
      input_cond[i] = (InY[i] - InX[i] <= delta)
      input_cond[i + l0_n] = (InX[i] - InY[i] <= delta)

    # Create hidden layer constraints
    L1Y, hl1_cond = vmmul_consume_sign(InY, hl1_sign, W1, B1, l0_n, l1_n, True)
